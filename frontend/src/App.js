...
const pathParts = (typeof window !== "undefined" ? window.location.pathname : "/")
  .split("/")
  .filter(Boolean);
const namespaceBase = pathParts.length > 0 ? `/${pathParts[0]}` : "";
const CORE = process.env.REACT_APP_CORE_BASE ||
  (typeof window !== "undefined" ? `${window.location.origin}${namespaceBase}/core` : "http://localhost:8080");
const ANALYSIS = process.env.REACT_APP_ANALYSIS_BASE ||
  (typeof window !== "undefined" ? `${window.location.origin}${namespaceBase}/analysis` : "http://localhost:8000");
...
export default function App() {
  ...
  async function refreshPoints() {
    ...
    setPoints(arr);
    setDataMessage(arr.length === 0 ? "Nessun dato live disponibile: il backend sta usando i fallback oppure la cache non è ancora popolata." : "");
  }
  ...
  return (
    <div>
      {dataMessage && <div style={{background:"#fff3cd", color:"#856404", padding:"10px", textAlign:"center", fontWeight:600}}>{dataMessage}</div>}
